...
class MarvelDetailDataEntityTests: XCTestCase {
    func testToDomain() {
        guard let mockData = getMock(name: "MarvelDetailDataEntityMock") else {
            XCTFail("Expected mock Data")
            return
        }
        do {
            let marvelListEntity = try JSONDecoder().decode(MarvelDetailDataEntity.self, from: mockData)
    ...
    func testInvalidName() throws {
        guard let mockData = getMock(name: "MarvelDetailEntityInvalidName") else {
            XCTFail("Expected mock Data")
            return
        }
        XCTAssertThrowsError(try JSONDecoder().decode(MarvelDetailDataEntity.self, from: mockData)) { error in
...
extension MarvelDetailDataEntityTests {
